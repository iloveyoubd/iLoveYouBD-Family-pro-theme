<?php
/**
 * ILYBD Dynamic SMS & Status Thumbnail Image Generator
 * Generates an ultra-premium cyberpunk / neon style featured image specifically for SMS
...
if (file_exists($wp_load_path)) {
    ...
    imagestring($im, 5, 20, 20, "ILoveYouBD SMS Engine", $text_col);
    imagepng($im);
    imagedestroy($im);
    exit;
}
...
if (!$post_id) {
    render_default_placeholder("iLoveYouBD.com", "SMS Collection");
    exit;
}

$post = get_post($post_id);
if (!$post || $post->post_type !== 'ilybd_sms') {
    render_default_placeholder("iLoveYouBD.com", "SMS Not Found");
    exit;
}
...
$theme = [
    ...
    'accent' => [0, 255, 102],      // Neon Emerald Green
    'bg' => [8, 19, 31],            // Cosmic deep space slate
    'bg_gradient' => [4, 8, 15],    // Dark cyber shadow
    'badge' => 'SMS CENTRAL HUB'
];
...
$gray = imagecolorallocate($im, 130, 150, 160);
$card_bg = imagecolorallocatealpha($im, 6, 12, 22, 25); 
...
$overlay_color = imagecolorallocate($overlay, 6, 12, 22);
imagefill($overlay, 0, 0, $overlay_color);
imagecopymerge($im, $overlay, 0, 0, 0, 0, $width, $height, 40);
imagedestroy($overlay);

// 7. Centered Glass box (Keep content perfectly inside mobile-safe center boundaries)
$box_x1 = 190;
$box_y1 = 80;
$box_x2 = 1010;
$box_y2 = 550;
...
// Subtle decorative digital messaging crosshair icon inside glassbox center
$cross_col = imagecolorallocatealpha($im, $theme['accent'][0], $theme['accent'][1], $theme['accent'][2], 80);
imageline($im, $box_x1 + 10, 100, $box_x1 + 30, 100, $cross_col);
imageline($im, $box_x1 + 20, 90, $box_x1 + 20, 110, $cross_col);
...
// Post Title ( Bengali Status )
$max_text_width = 740;
$title_font_size = 36;

if (file_exists($font_path)) {
    $title_lines = wrap_sms_text($title_font_size, 0, $font_path, $title, $max_text_width);
...
$watermark_text = "ILOVEYOUBD.COM  |  NEXT-GEN SMS GATEWAY";
$sub_watermark = "Verified Dynamic OpenGraph Card for Real-Time Social Distribution";
...
function wrap_sms_text($fontSize, $angle, $fontFace, $string, $maxWidth) {
    $lines = [];
